...
public class PlayerScheduleViewEndpointTest
{
    [Fact]
    public async Task ViewManagerOverview_ShouldReturnListOfDailyScheduleForMonth()
    {
        ...
        // Act
        var response = await client.GetAsync($"/api/player/schedule?MonthName={monthName}");
    ...
    public async Task ViewManagerOverview_ShouldReturnBadRequest_WhenMonthNameIsMissing()
    {
        ...
        // Act
        var response = await client.GetAsync("/api/player/schedule");
